 ...
 *
 * Used the customize setting options on fourteenth template,
 ...
 */	
	...
	if(get_theme_mod( 'medma_matix_sp_14_background_img' ) !== false && get_theme_mod( 'medma_matix_sp_14_background_img' ) !== ''){
		$background_img = get_theme_mod( 'medma_matix_sp_14_background_img' );
	}else{
		$background_img = plugins_url('../img/temp14.jpg', __FILE__ );
	}
	
	if(get_theme_mod( 'medma_matix_sp_14_background_color' ) !== false){
		$background_color = get_theme_mod( 'medma_matix_sp_14_background_color' );
	}else{
		$background_color = '#58EF98';
	}
	
	
	if(get_option( 'medma_matix_temp14_con_1' ) !== false){
		$content1 = get_option( 'medma_matix_temp14_con_1' );
	}else{
		$content1 = "JOIN US";
	}
	
	if(get_option( 'medma_matix_temp14_con_2' ) !== false){
		$content2 = get_option( 'medma_matix_temp14_con_2' );
	}else{
		$content2 = "Deals Everyday";
	}
	if(get_option( 'medma_matix_temp14_con_3' ) !== false){
		$content3 = get_option( 'medma_matix_temp14_con_3' );
	}else{
		$content3 = "Subscribe";
	}
...
<?php 
	wp_enqueue_style( 'temp-14-quills-css', plugins_url('../../public/css/quill.snow.css', __FILE__ ));
?>
...
<?php }?>
	...
	<div class="temp-14" style="background-image: url('<?php echo $background_img;?>');">
		<div class="temp-14-main" style="background-color: <?php echo $background_color;?>;">
			<h2 class="temp-14-tag" id="editor1" data-editor="1" style="margin-bottom: 0px;"><?php echo $content1; ?></h2>
			<h3 class="temp-14-subtag" id="editor2" data-editor="2" style="margin-top: 0px;"><?php echo $content2; ?></h3>
			<h5 class="temp-14-subscribe" id="editor3" data-editor="3"><?php echo $content3; ?></h5>
			<!--div class="link-text">	<a class="temp14_link" href="<!?php echo $button_link ;?>" style="color:<!?php echo $text_color; ?>"><!?php echo $content4;?></a></div-->
			<div class="link-text">	
				<?php if( is_customize_preview() ){?>
					<p class="temp14_link">+ Newsletter goes here.</p>
				...
				<?php } else{ ?>
					<form name="customer_details" id="customer_details" method="POST" onsubmit="return medma_mtx_form_validation('<?php echo admin_url( 'admin-ajax.php' ); ?>')" action="javascript:void(0);">
						...
						<input type="hidden" id="template-number" name="template_number" value="14"/>
		  ...
		  if (source == 'api') {
			if(checkAjax1 == ''){
				checkAjax1 = setTimeout(function(){ 
					...
					medma_mtx_setContentValue('medma_matix_temp14_con_1',content,1);
				}, 3000);
			}
		  } else if (source == 'user') {
		  ...
		  if (source == 'api') {
			if(checkAjax2 == ''){
				checkAjax2 = setTimeout(function(){ 
					...
					medma_mtx_setContentValue('medma_matix_temp14_con_2',content,2);
				}, 3000);
			}
		  } else if (source == 'user') {
		  ...
		  if (source == 'api') {
			if(checkAjax3 == ''){
				checkAjax3 = setTimeout(function(){ 
					...
					medma_mtx_setContentValue('medma_matix_temp14_con_3',content,3);
				}, 3000);
			}
		  } else if (source == 'user') {
