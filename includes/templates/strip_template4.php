 ...
 *
 * Used the customize setting options on fourth strip template
 ...
 */
	...
	if(get_theme_mod( 'medma_matix_sp_strip_4_back_image' ) !== false){
		$back_color = get_theme_mod( 'medma_matix_sp_strip_4_back_image' );
	}else{
		$back_color = esc_url( plugins_url( '../img/red-sequin-background.jpg', __FILE__ ) );
	}
	
	if(get_theme_mod( 'medma_matix_sp_strip_4_button_color' ) !== false){
		$button_color = get_theme_mod( 'medma_matix_sp_strip_4_button_color' );
	}else{
		$button_color = '#FF9900';
	}
	
	if(get_theme_mod( 'medma_matix_sp_strip_4_button_text' ) !== false){
		$button_text = get_theme_mod( 'medma_matix_sp_strip_4_button_text' );
	}else{
		$button_text = 'SHOP NOW';
	}
	
	if(get_theme_mod( 'medma_matix_sp_strip_4_button_text_color' ) !== false){
		$button_text_color = get_theme_mod( 'medma_matix_sp_strip_4_button_text_color' );
	}else{
	...
	if(get_option( 'medma_matix_strip4_con_1' ) !== false){
		$content1 = get_option( 'medma_matix_strip4_con_1' );
	}else{
		$content1 = 'Christmas Discounts! Upto <b><i>50% OFF!</i></b> This weekend only!';
	}
...
<div class="strip-temp-4" style="background-image:url('<?php echo $back_color;?>');">
	<div class="strip-temp-4-shop" id="strip_editor1" data-editor="1"><?php echo $content1;?></div>
	<a href="javascript:void(0);" id="strip-template-button" style="display:<?php echo $btn_sts;?>;color:<?php echo $button_text_color;?>;background-color:<?php echo $button_color;?>;"><?php echo $button_text;?></a> 
	<?php echo $close_icon;?>
</div>
		  ...
		  if (source == 'api') {
			if(s_checkAjax1 == ''){
				s_checkAjax1 = setTimeout(function(){ 
					...
					medma_mtx_s_setContentValue('medma_matix_strip4_con_1',s_content,1);
				}, 3000);
			}
		  } else if (source == 'user') {
