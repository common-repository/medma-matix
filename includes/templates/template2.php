 ...
 *
 * Used the customize setting options on second template
 ...
 */
	...
	if(get_theme_mod( 'medma_matix_sp_2_button_color' ) !== false){
		$button_color = get_theme_mod( 'medma_matix_sp_2_button_color' );
	}else{
		$button_color = '#FB0169';
	}
	
	if(get_theme_mod( 'medma_matix_sp_2_button_text' ) !== false && get_theme_mod( 'medma_matix_sp_2_button_text' ) !== ''){
		$button_text = get_theme_mod( 'medma_matix_sp_2_button_text' );
	}else{
		$button_text = 'SHOP NOW';
	}
	
	if(get_theme_mod( 'medma_matix_sp_2_text_color' ) !== false && get_theme_mod( 'medma_matix_sp_2_text_text' ) !== ''){
		$button_text_color = get_theme_mod( 'medma_matix_sp_2_text_color' );
	}else{
		$button_text_color = 'ffffff';
	}
	
	if(get_theme_mod( 'medma_matix_sp_2_button_link' ) !== false && get_theme_mod( 'medma_matix_sp_2_button_link' ) !== ''){
		$button_link = get_theme_mod( 'medma_matix_sp_2_button_link' );
	}else{
		$button_link = 'javascript:void(0);';
	}
	
	if(get_option( 'medma_matix_temp2_con_1' ) !== false){
		$content1 = get_option( 'medma_matix_temp2_con_1' );
	}else{
		$content1 = 'Independence Day Sale';
	}
	
	if(get_option( 'medma_matix_temp2_con_2' ) !== false){
		$content2 = get_option( 'medma_matix_temp2_con_2' );
	}else{
		$content2 = 'GET 30% OFF';
	}
	
	if(get_option( 'medma_matix_temp2_con_3' ) !== false){
		$content3 = get_option( 'medma_matix_temp2_con_3' );
	}else{
		$content3 = 'Coupon Code - GET30';
	}
	
	if(get_option( 'medma_matix_temp2_con_4' ) !== false){
		$content4 = get_option( 'medma_matix_temp2_con_4' );
	}else{
		$content4 = '*Terms & Conditions Apply';
	}
...
<?php 
	wp_enqueue_style( 'temp-2-quills-css', plugins_url('../../public/css/quill.snow.css', __FILE__ ));
?>
...
<?php }?>
	...
	<div class="temp-2-main">
		<div class="temp-2-independence" id="editor1" data-editor="1"><?php echo $content1;?></div>
		<h2 id="editor2" data-editor="2"><b><?php echo $content2;?></b></h2>
		<div class="temp-2-coupon" id="editor3" data-editor="3"><?php echo $content3;?></div>
		<div class="temp-2-shop" style="background-color: <?php echo $button_color;?>;">
			<a href="<?php echo $button_link;?>" target="_blank" style="color:<?php echo $button_text_color;?>"><?php echo $button_text;?></a>
		</div>
		<div class="temp-2-terms" id="editor4" data-editor="4"><?php echo $content4;?></div>
	</div>
		  ...
		  if (source == 'api') {
			if(checkAjax1 == ''){
				checkAjax1 = setTimeout(function(){ 
					...
					medma_mtx_setContentValue('medma_matix_temp2_con_1',content,1);
				}, 3000);
			}
		  } else if (source == 'user') {
		  ...
		  if (source == 'api') {
			if(checkAjax2 == ''){
				checkAjax2 = setTimeout(function(){ 
					...
					medma_mtx_setContentValue('medma_matix_temp2_con_2',content,2);
				}, 3000);
			}
		  } else if (source == 'user') {
		  ...
		  if (source == 'api') {
			if(checkAjax3 == ''){
				checkAjax3 = setTimeout(function(){ 
					...
					medma_mtx_setContentValue('medma_matix_temp2_con_3',content,3);
				}, 3000);
			}
		  } else if (source == 'user') {
		  ...
		  if (source == 'api') {
			if(checkAjax4 == ''){
				checkAjax4 = setTimeout(function(){ 
					...
					medma_mtx_setContentValue('medma_matix_temp2_con_4',content,4);
				}, 3000);
			}
		  } else if (source == 'user') {
